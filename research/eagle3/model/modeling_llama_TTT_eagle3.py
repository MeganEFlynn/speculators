    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        for past_hidden_state in past_hidden_states:
            key_cache.append(
                self.k_proj(past_hidden_state).view(hidden_shape).transpose(1, 2)
            )
            _, key_cache[-1] = apply_rotary_pos_emb(
                query_states, key_cache[-1], cos, sin
            )
            value_cache.append(
                self.v_proj(past_hidden_state).view(hidden_shape).transpose(1, 2)
            )
